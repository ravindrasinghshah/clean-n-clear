...
function RoutineSection({ title, label: sectionLabel, steps }: { title: string; label: string; steps: RoutineRecommendation['morning'] }) {
  const stepsWithProducts = steps as RoutineStepWithProducts[];
  const hasProductRecommendations = stepsWithProducts.some((step) => step.products && step.products.length > 0);
