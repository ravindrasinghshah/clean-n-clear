...
const initialPreferences: ScanPreferences = {
  routineLevel: "standard",
...
const loadingGifSrc = "/images/loading.gif";
...
export default function ScanPage() {
  ...
  const actionDisabled =
    !image || loading || preparingImage || Boolean(retakeFeedback);
  ...
  async function analyze() {
    ...
    try {
      ...
      if (
        response.status === 422 &&
        (data.status === "retake" ||
          data.code === "IMAGE_QUALITY_INSUFFICIENT" ||
          imageQuality)
      ) {
        setRetakeFeedback({
          imageQuality,
      ...
      if (!data.analysis || !data.routine) {
        setError(
          "We could not read this scan. Please upload another photo and try again.",
        );
        return;
      }
      ...
      setError(
        "We could not reach the analysis service. Please check your connection and try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="min-h-[100dvh] bg-white">
      <div className="mx-auto flex min-h-[100dvh] w-full max-w-[430px] flex-col">
        <div className="px-5 pb-36 pt-5">
          <header className="flex items-center justify-between gap-4">
            <Link
              className="inline-flex items-center gap-2 rounded-full border border-ink/10 px-3 py-2 text-sm font-semibold text-ink transition-colors hover:bg-accent-soft"
              href="/"
            >
            ...
            <span className="rounded-full bg-accent-soft px-3 py-2 text-xs font-bold text-ink">
              Step 1 of 2
            </span>
          </header>

          <section
            className="mt-6 rounded-[2rem] bg-accent-soft p-5"
          ...
          >
            ...
            <h1
              id="scan-title"
              className="mt-3 text-[2.15rem] font-bold leading-[1.04] tracking-[-0.05em] text-ink"
            >
              Let&apos;s start with a clear selfie.
            </h1>
            <p className="mt-3 max-w-[33ch] text-[0.98rem] leading-6 text-ink/70">
              We only build a routine when we can clearly read the photo, so you
              can trust the next steps.
            </p>
            <div
          ...
          <section className="mt-8" aria-labelledby="photo-title">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-5 text-ink/70">
                  A front-facing photo in even light gives the most useful
                  result.
                </p>
              </div>
            </div>
            ...
            <div className="mt-4">
              <FaceCapture
                ...
                onPreparationChange={setPreparingImage}
              />
            </div>

            <aside
              className="mt-4 rounded-[1.5rem] border border-accent/25 bg-accent-soft p-4"
              aria-labelledby="photo-tips-title"
            >
              <h3 id="photo-tips-title" className="text-sm font-bold text-ink">
              ...
              <ul className="mt-3 space-y-2 text-sm leading-5 text-ink/70">
                <li className="flex gap-2">
                  ...
                  <span>
                    Use bright, even daylight and avoid strong shadows or
                    backlighting.
                  </span>
                </li>
                <li className="flex gap-2">
                  ...
                  <span>
                    Keep your full face centered, at eye level, and looking
                ...
                <li className="flex gap-2">
                  ...
                  <span>
                    Skip filters and remove anything that hides your skin.
                  </span>
                </li>
              </ul>
            </aside>
          ...
          <section className="mt-8" aria-labelledby="preferences-title">
            <div className="flex items-start gap-3">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-ink/20 bg-white text-sm font-bold text-ink">
                2
              </span>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-ink/70">
                ...
                <p className="mt-1 text-sm leading-5 text-ink/70">
                  Choose the type of routine and goals you want to focus on.
                </p>
              </div>
            </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            {error && (
              <p
                className="rounded-2xl border border-accent/30 bg-accent-soft p-4 text-sm text-ink"
                role="alert"
              >
                {error}
              </p>
      ...
      {loading && (
        <div
          className="fixed inset-0 z-40 flex items-center justify-center bg-ink/30 px-5"
          role="status"
          aria-live="polite"
        >
          <div className="w-full max-w-[360px] rounded-[2rem] bg-white p-6 text-center shadow-soft">
            <img
              ...
              aria-hidden="true"
            />
            <p className="mt-5 text-xl font-bold tracking-tight text-ink">
      ...
      <div className="fixed inset-x-0 bottom-0 z-20">
        <div className="mx-auto w-full max-w-[430px] border-t border-ink/10 bg-white/95 px-5 pb-[calc(env(safe-area-inset-bottom)+0.25rem)] pt-3 backdrop-blur">
          <button
            ...
            onClick={analyze}
            className="w-full rounded-2xl bg-accent px-6 py-4 font-bold text-ink shadow-soft transition-colors hover:bg-accent/80 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {actionLabel}
          </button>
...
function RetakeGuidance({ feedback }: { feedback: RetakeFeedback }) {
  const requestedInstructions =
    feedback.tips?.filter(Boolean) ??
    feedback.imageQuality?.retakeInstructions?.filter(Boolean);
  const instructions = requestedInstructions?.length
    ? requestedInstructions
    : defaultRetakeInstructions;
  const issues =
    feedback.reasons?.filter(Boolean) ??
    feedback.imageQuality?.issues?.filter(Boolean) ??
    [];
  const confidence =
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold uppercase tracking-wide text-ink">
        A clearer photo is needed
      </p>
      <h3 className="mt-1 text-xl font-bold">
        Let&apos;s retake this before building your routine.
      </h3>
      <p className="mt-2 text-sm text-ink/70">
        We only create recommendations from photos we can read confidently.
        Upload a new selfie using the tips below.
        {confidence !== undefined &&
          ` This scan was ${confidence}% confident; we need at least ${Math.round(minimumConfidence * 100)}%.`}
      </p>
      {issues.length > 0 && (
        <div className="mt-3">
          <p className="text-sm font-semibold text-ink">
            What made this photo difficult to read
          </p>
          <ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-ink/70">
            {issues.map((issue) => (
              <li key={issue}>{qualityIssueMessage(issue)}</li>
            ))}
          </ul>
        </div>
      )}
...
function qualityIssueMessage(issue: ImageQualityIssue) {
  const messages: Record<ImageQualityIssue, string> = {
    ...
    "face-not-front-facing":
      "Your face was not looking straight toward the camera.",
    ...
    "insufficient-detail":
      "There was not enough visible skin detail in this photo.",
  };
